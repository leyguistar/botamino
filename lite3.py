...
def checkChat(chatid,comid,message,host,cohosts):
                        ...
                        if(comando in interaciones1):
                            ...
                            if(len(content) == 2):
                                ...
                                if(mn < len(marcos)   and mn >= 0):
                                    ...
                                    send_message(chatid,'La version lite del bot solo puede usar los marcos del 0 al %d' % (len(marcos) ) )
                            elif(len(content) == 3):
                                mn = int(content[1])
                                mn2 = int(content[1])
                            ...
                            if(m):
                                if(len(m) > 20):
                                    send_message(chatid,'No se pueden decir textos de mas de 20 letras')
                                else:
                                    for i in range(3):
                                        try:
                            ...
                            chatStates[chatid] = 0
                            chatStateLock.release()
                    
                            return
                        elif(comando == 'startbot'):
                            content = message['content'].split(' ')
                            if(len(content) > 1 ):
                                r = checkStartBot(chatid,userid,comid,id,' '.join(content[1:]) )
                            else:
                                r = checkStartBot(chatid,userid,comid,id )
                            if(r == 1):
                                chatStateLock.acquire()
                                s.botstate(0,0,comid,0,chatid)
                                send_message(chatid,'apagando bot lite')
...
def connectServer():
    ...
    while True:
        try:
            ...
            sleep(60)
def checkStartBot(chatid,uid,cid,replyid,alias=None):
...
def on_group_member_join(data):
    ...
    if(chatid not in chats):
        ...
        chat = chats[chatid]
    if(chatid in chatStates and chatStates[chatid] == 2):
...
def checkStart(data):
    ...
    if(chatThreads[chatid]['type'] == 0):
        print('enviando a privado')
        lite_private.checkPrivate(chatid,message,client,cid)
        return
    # print(data)
    content = message['content']
    userid = message['uid']
    ...
    if(chatid not in chatStates):
        ...
        dblock.release()
        chatStates[chatid] = 0
    elif(chatStates[chatid] == 2):
        try:
            checkChat(chatid,cid,message,get_host(chatid,cid),get_cohosts(chatid,cid))
        except Exception as e:
            print(e)
            PrintException()
    elif(chatStates[chatid] == 0):
        if(content == '/activar' and (userid == get_host(chatid,cid) or userid == ley) ):
            comidChat[chatid] = cid
            if(test and chatid != '17ab60ec-0418-4c75-8ce3-fdb11613b201' and chatid != '783edbe5-9782-46f7-b5b7-fc42c2ae4875'):
                print('se quiso activar el bot en',chatid)
            else:
                dblock.acquire()
        ...
        elif(content.startswith( '/startbot')
            and (get_host(chatid,cid) == message['uid'] or message['uid'] == ley)):
            if(test and chatid != '17ab60ec-0418-4c75-8ce3-fdb11613b201' and chatid != '783edbe5-9782-46f7-b5b7-fc42c2ae4875'):
                print('se quiso activar el bot con startbot en',chatid)
            else:        
                content = message['content'].split(' ')
                if(len(content) > 1 ):
                    r = checkStartBot(chatid,message['uid'],cid,id,' '.join(content[1:]) )
                else:
                    r = checkStartBot(chatid,message['uid'],cid,id)
                if(r == 0):
    ...
    while 1:
        ...
        for comid in comids:
            ...
            for chat in threads:
                ...
                # chatHost[chatid] = chat['uid']
                # if('coHost' in chat['extensions']):
                #     chatCoHosts[chatid] = chat['extensions']['coHost']
                # else:
                #     chatCoHosts[chatid] = []
...
def updateStates():
    s2 = Save(file=sqlCredentials)
    chatStateLock.acquire()
    s2.cursor.execute('select chatid,state,comid from botstate;')
    for r in s2.cursor.fetchall():
        chatStates[r[0]] = r[1]
        comidChat[r[0]] = r[2]
    chatStateLock.release()
...
def handler(signum, frame):
    print('se recibio sigsevg a las ',time())
    print('signum',signum)
    print('frame',frame)

signal.signal(signal.SIGSEGV, handler) #handler para matar cuando nazca un nuevo bot
...
tChat = threading.Thread(target=checkStates, args=(600,))
tChat.daemon = True
tChat.start()
...
# tChat = threading.Thread(target=reiniciar, args=(600,))
# tChat.daemon = True
# tChat.start()
...
while 1:
    ...
    if(text == 'threads'):
        ...
        try:
            dblock.release()
            print('released dblock')
        except Exception as e:
            print(e)
        try:
            chatStateLock.release()
            print('released chatStateLock')
        except Exception as e:
            print(e)
        try:
            oldMessagesLock.release()
