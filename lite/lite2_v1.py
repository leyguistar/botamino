...
print('iniciado')
def checkComunity(comid,s):
    sub_client = amino.SubClient(comId=comid,profile=client.profile)
    while True:
        try:
            readChats = sub_client.get_chat_threads(start=0, size=40,raw=True)  # Gets all chats the user is in
            for js in readChats['threadList']:
                ...
                if(chatid not in chatsRevisando):
                    ...
                    if(chatid not in lastActivity):
                        ...
                        continue
                    if(chatLastActivity <= lastActivity[chatid]):
                        continue
                    lastActivity[chatid] = chatLastActivity
                    if(state == 0):
...
for cid in comids:
    tChat = threading.Thread(target=checkComunity, args=(cid,Save()))
    tChat.daemon = True
    tChat.start()
while 1:
    dblock.acquire()
